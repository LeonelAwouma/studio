    ...
    <header className="sticky top-0 z-50 w-full glass border-b border-white/5 transition-all duration-300">
      <div className="container flex h-20 items-center">
        ...
        <nav className="hidden lg:flex items-center space-x-10 text-sm font-semibold tracking-wide">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="text-foreground/70 transition-all hover:text-primary hover:scale-105"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="flex flex-1 items-center justify-end gap-6">
          <LocaleSwitcher />
          <Button asChild className="hidden md:flex rounded-full px-6 glow-shadow">
            ...
            <SheetContent side="right" className="glass border-white/10 w-[300px] sm:w-[400px]">
              ...
              <nav className="flex flex-col gap-8 text-xl font-bold mt-12 px-4">
                ...
                {navLinks.map((link) => (
                 ...
                 <Button asChild className="mt-10 rounded-full h-14 text-lg">
                  <a href="/CV_LEONEL_AWOUMA.pdf" download>{t('downloadCV')}</a>
                </Button>
              </nav>
            </SheetContent>
